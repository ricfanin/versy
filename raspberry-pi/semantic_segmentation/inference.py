...
def postprocess(outputs, frame_h, frame_w, img_size, conf_threshold=0.5):
    """Decodifica output YOLOv8-seg: NMS + ricostruzione maschere dai prototipi."""
    det_output = outputs[0]  # (1, 4+nc+32, N)
    proto = outputs[1]       # (1, 32, mask_h, mask_w)

    predictions = det_output[0].T  # (N, 4+nc+32)
    ...
    confidences = class_scores[:, 0] if num_classes == 1 else np.max(class_scores, axis=1)

    above_thresh = confidences > conf_threshold
    if not np.any(above_thresh):
        return np.zeros((frame_h, frame_w), dtype=np.uint8)

    boxes = boxes[above_thresh]
    confidences = confidences[above_thresh]
    mask_coeffs = mask_coeffs[above_thresh]
    ...
    indices = cv2.dnn.NMSBoxes(
        boxes_xyxy.tolist(), confidences.tolist(), conf_threshold, 0.5
    )
    if len(indices) == 0:
        return np.zeros((frame_h, frame_w), dtype=np.uint8)
    ...
    # Ricostruisci maschere dai prototipi
    ...
    for idx in indices:
        raw_mask = np.tensordot(mask_coeffs[idx], proto_masks, axes=([0], [0]))
